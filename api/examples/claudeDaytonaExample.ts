...
async function ensureDevServerRunning(sandbox: Sandbox) {
  ...
  try {
    ...
    const statusCheck = await sandbox.process.executeCommand(
    ...
    if (statusCheck.exitCode === 0) {
      ...
      if (viteProcess?.pm2_env?.status === 'online') {
        ...
        console.log(
          `Dev server process exists but status is: ${viteProcess?.pm2_env?.status}, restarting...`
        );
...
async function getOrCreateSandbox(
  ...
  if (sandboxState.sandboxId) {
    try {
      console.log(
        `Attempting to find existing sandbox with ID: ${sandboxState.sandboxId}`
      );
      ...
      if (state === 'STARTED') {
        ...
        try {
          ...
          if (healthCheck.ok) {
            ...
            console.log(
              `Dev server not responsive (status: ${healthCheck.status}), ensuring it's running...`
            );
          ...
          console.log(
            "Dev server health check failed, ensuring it's running..."
          );
          await ensureDevServerRunning(sandbox);
          return sandbox;
        }
        ...
        await sandbox.start();
        console.log(
          'Sandbox started successfully, ensuring dev server is running...'
        );
        await ensureDevServerRunning(sandbox);
        ...
        console.log(
          'Sandbox is archived, starting it (this may take longer)...'
        );
        await sandbox.start();
        console.log(
          'Archived sandbox started successfully, ensuring dev server is running...'
        );
        await ensureDevServerRunning(sandbox);
        ...
        console.log(
          `Sandbox is in unexpected state: ${sandbox.state}, creating new sandbox`
        );
...
async function getSandboxManager(env: Env) {
  const sandboxManagerId = env.SANDBOX_MANAGER.idFromName(
    'nightona-sandbox-state'
  );
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/initialize' && request.method === 'POST') {
      try {
        ...
        await sandbox.process.executeCommand(
          'id claude || useradd -m -s /bin/bash claude'
        );
        ...
        await sandbox.process.executeCommand(
          'cp -r /workspace/template /tmp/project'
        );
        ...
        await sandbox.process.executeCommand(
          'chown -R claude:claude /tmp/project'
        );
        ...
        return Response.json({
          success: true,
          message:
            'Project initialized successfully with React + TypeScript template',
          ...
          return Response.json(
            {
              ...
              details:
                "Run 'npm run create-snapshot' to create the required snapshot before starting the service",
    ...
    if (url.pathname === '/api/run-code' && request.method === 'POST') {
      try {
        ...
        try {
          ...
          aiResponse =
            ('result' in claudeResult ? claudeResult.result : '') ||
            response.result;
